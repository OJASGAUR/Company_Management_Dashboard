    ...
    <div className="flex h-full flex-col bg-slate-950 text-slate-100">
      <div className="flex h-20 items-center justify-between border-b border-slate-900 px-6">
        <Link href="/dashboard" className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 font-black text-white shadow-md shadow-indigo-500/20">C</div>
          <div><p className="text-sm font-extrabold tracking-tight text-white">Company OS</p><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-indigo-400">Workspace</p></div>
        </Link>
      ...
      <div className="border-t border-slate-900 p-4">
        <Link href="/dashboard/profile" onClick={onCloseMobile} className="mb-3 flex items-center gap-3 rounded-xl p-2 transition-colors hover:bg-slate-900">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-800 text-sm font-bold text-white shadow-sm">{user.name?.charAt(0).toUpperCase() || user.email?.charAt(0).toUpperCase() || "U"}</div>
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-white">{user.name || "User Profile"}</p><p className="truncate text-xs text-slate-400">{role.replace(/_/g, " ")}</p></div>
          <span className="text-slate-500">→</span>
        </Link>
        <form action={signOutAction}><button type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl border border-rose-900/30 bg-rose-950/20 px-4 py-2.5 text-xs font-semibold text-rose-400 transition-colors hover:bg-rose-900/40 hover:text-rose-300"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>Sign Out</button></form>
      </div>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div className="flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white px-4 lg:hidden">
        <Link href="/dashboard" className="flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-600 font-bold text-white">C</div><span className="font-bold text-slate-900">Company OS</span></Link>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-slate-700 hover:bg-slate-100" aria-label="Open Navigation Menu"><svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg></button>
        </div>
      </div>
