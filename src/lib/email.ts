...
export async function sendNotificationEmail({
  ...
  const safeName = recipientName || "Team Member"
  ...
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table width="100%" style="max-width:600px;background:#fff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden" cellpadding="0" cellspacing="0">
      ...
      <tr><td style="padding:28px">
        ...
        <div style="font-size:12px;font-weight:700;color:#1d4ed8;text-transform:uppercase;margin-bottom:8px">${icon} ${type.replace(/_/g, " ")}</div>
        <h1 style="font-size:24px;line-height:1.3;margin:0 0 16px;color:#0f172a">${title}</h1>
        <div style="background:#f8fafc;border-left:4px solid #3b82f6;border-radius:8px;padding:16px 18px;line-height:1.6;color:#334155">${message}</div>
        <div style="text-align:center;margin-top:28px"><a href="${fullLink}" style="display:inline-block;background:#2563eb;color:#fff;text-decoration:none;font-weight:700;padding:12px 22px;border-radius:10px">View Notification</a></div>
      </td></tr>
