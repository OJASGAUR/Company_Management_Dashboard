...
export default async function ChatPage() {
  ...
  const currentUser = await prisma.user.findUnique({ where: { id: session.user.id }, select: { id: true, name: true, role: true } })
  if (!currentUser) redirect("/")
  if (currentUser.role === "CLIENT") redirect("/dashboard/client/messages")

  const [otherUsers, messages, groups] = await Promise.all([
    prisma.user.findMany({ where: { id: { not: currentUser.id }, isActive: true }, select: { id: true, name: true, role: true }, orderBy: { name: "asc" } }),
    ...
    prisma.messageGroup.findMany({ where: { members: { some: { userId: currentUser.id } } }, include: { members: { include: { user: { select: { id: true, name: true, role: true } } } } }, orderBy: { createdAt: "desc" }, take: 100 }),
  ])
