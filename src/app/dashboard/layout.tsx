...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const isManagement: boolean = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.HR, Role.OPERATIONS_MANAGER, Role.ACCOUNTS].includes(role)
  const canManageAssets: boolean = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.HR, Role.OPERATIONS_MANAGER].includes(role)
  const canSeeFinance: boolean = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.ACCOUNTS].includes(role)
  const canSeeReports: boolean = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.HR, Role.OPERATIONS_MANAGER, Role.ACCOUNTS].includes(role)
  const canSeeProjects: boolean = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.OPERATIONS_MANAGER, Role.TEAM_LEAD, Role.DEVELOPER, Role.DESIGNER, Role.TESTER].includes(role)
