...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
      ...
      <div className="flex h-screen bg-slate-50 text-slate-900">
        <aside className="flex w-72 flex-col bg-slate-950 text-white">
          <div className="border-b border-slate-800 p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400">Client Portal</p>
            <h2 className="mt-1 text-2xl font-bold tracking-tight">Company OS</h2>
            <p className="mt-1 text-xs text-slate-500">Limited client workspace</p>
          </div>
          <nav className="flex-1 space-y-1 overflow-y-auto px-4 py-5">
            <p className="mb-2 px-4 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Workspace</p>
            <Link href="/dashboard/client" className={navClass}>Overview</Link>
            <Link href="/dashboard/client/projects" className={navClass}>Projects</Link>
            <Link href="/dashboard/client/tasks" className={navClass}>Project Tasks</Link>
            <Link href="/dashboard/client/invoices" className={navClass}>Invoices & Billing</Link>
            <Link href="/dashboard/client/documents" className={navClass}>Documents</Link>
            <Link href="/dashboard/client/calendar" className={navClass}>Calendar & Milestones</Link>
            <Link href="/dashboard/client/messages" className={navClass}>Messages</Link>
          ...
          <div className="border-t border-slate-800 p-4">
            <div className="mb-4 flex items-center gap-3 px-2"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-cyan-600 text-sm font-bold">{user.email?.charAt(0).toUpperCase() || "C"}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{user.name || "Client"}</p><p className="truncate text-xs text-slate-500">Client Portal</p></div></div>
            <form action={async () => { "use server"; await signOut({ redirectTo: "/" }) }}><button type="submit" className="w-full rounded-lg px-4 py-2.5 text-left text-sm font-medium text-red-400 transition hover:bg-slate-800">Sign Out</button></form>
          </div>
        </aside>
  ...
  const canSeeProjects = ["SUPER_ADMIN", "DIRECTOR", "OPERATIONS_MANAGER", "TEAM_LEAD", "DEVELOPER", "DESIGNER", "TESTER"].includes(role)
