...
export default async function ReportsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7">
      ...
      <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-6"><h2 className="font-semibold text-indigo-950">Next analytics layer</h2><p className="mt-2 text-sm leading-6 text-indigo-800">The reporting foundation is now connected to live database counts. Department productivity, attendance trends, revenue, project profitability and monthly dashboards can be layered onto the same reporting surface as those data models mature.</p></div>
    </div>
  )
}
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="mb-5 text-lg font-semibold text-slate-900">{title}</h2>{children}</section> }
