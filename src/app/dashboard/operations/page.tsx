...
export default async function OperationsDashboard() {
  ...
  if (!["SUPER_ADMIN", "DIRECTOR", "OPERATIONS_MANAGER"].includes(session.user.role)) redirect("/unauthorized")

  const [employees, projects, activeTasks] = await Promise.all([
    prisma.user.findMany({ where: { role: { notIn: ["SUPER_ADMIN", "CLIENT"] }, isActive: true }, select: { id: true, name: true, role: true, department: true } }),
