...
export async function punchIn() {
  ...
  revalidatePath("/dashboard"); revalidatePath("/dashboard/attendance"); return { success: true }
}
...
export async function getAttendanceHistory() {
  ...
  if (user.role === Role.SUPER_ADMIN) {
    return prisma.attendance.findMany({ include: { user: { select: { id: true, name: true, email: true, employeeId: true, department: true } } }, orderBy: { date: "desc" }, take: 500 })
  }
  return prisma.attendance.findMany({ where: { userId: user.id }, orderBy: { date: "desc" }, take: 30 })
}
...
export async function updateTaskStatus(taskId: string, newStatus: string) {
  ...
  const canManageAllTasks = await (async () => { try { await requirePermission("assignTasks"); return true } catch { return false } })()
...
export async function assignTask(formData: FormData) {
  const actor = await requirePermission("assignTasks"), title = requiredString(formData.get("title"), "Title", 200)
...
export async function updateLeaveStatus(formData: FormData) {
  await requirePermission("approveLeaves")
  ...
  const leave = await prisma.leave.findUnique({ where: { id: leaveId }, select: { userId: true, type: true, startDate: true, endDate: true, status: true, user: { select: { id: true, isActive: true } } } })
  ...
  if (leave.user.isActive) {
    ...
    await createNotification({ userId: leave.userId, title: `Leave request ${statusLabel}`, message: `Your ${leave.type.toLowerCase()} leave request from ${leave.startDate.toLocaleDateString()} to ${leave.endDate.toLocaleDateString()} has been ${statusLabel}.`, type: "LEAVE", link: "/dashboard/leaves" })
  }
  revalidatePath("/dashboard/leaves"); revalidatePath("/dashboard")
}
...
export async function getNotifications() {
  const user = await requireAuth(); return prisma.notification.findMany({ where: { userId: user.id }, orderBy: { createdAt: "desc" }, take: 100 })
}
export async function getUnreadNotificationCount() { const user = await requireAuth(); return prisma.notification.count({ where: { userId: user.id, readAt: null } }) }
export async function markNotificationAsRead(notificationId: string) { const user = await requireAuth(); const safeId = id(notificationId, "Notification ID"); await prisma.notification.updateMany({ where: { id: safeId, userId: user.id }, data: { readAt: new Date() } }); revalidatePath("/dashboard/notifications"); revalidatePath("/dashboard"); return { success: true } }
export async function markAllNotificationsAsRead() { const user = await requireAuth(); await prisma.notification.updateMany({ where: { userId: user.id, readAt: null }, data: { readAt: new Date() } }); revalidatePath("/dashboard/notifications"); revalidatePath("/dashboard"); return { success: true } }
export async function deleteNotification(notificationId: string) { const user = await requireAuth(); const safeId = id(notificationId, "Notification ID"); await prisma.notification.deleteMany({ where: { id: safeId, userId: user.id } }); revalidatePath("/dashboard/notifications"); revalidatePath("/dashboard"); return { success: true } }

export async function updateNotificationPreferences(formData: FormData) {
  const user = await requireAuth();
  await prisma.user.update({ where: { id: user.id }, data: { emailTasks: formData.get("emailTasks") === "on", emailLeaves: formData.get("emailLeaves") === "on", emailAnnouncements: formData.get("emailAnnouncements") === "on" } });
  revalidatePath("/dashboard/notifications"); return { success: true }
}

export async function createNotification({ userId, title, message, type = "INFO", link }: { userId: string; title: string; message: string; type?: string; link?: string | null }) {
  await requireAuth(); const safeUserId = id(userId, "User ID"), safeTitle = requiredString(title, "Notification title", 200), safeMessage = requiredString(message, "Notification message", 4000), safeType = enumValue(type, "Notification type", NOTIFICATION_TYPES)
