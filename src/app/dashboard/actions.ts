...
import { LeaveStatus, LeaveType, ProjectStatus, Role, TaskPriority, TaskStatus } from "@prisma/client"
...
export async function getDashboardStats() {
  ...
  const isManager = [Role.SUPER_ADMIN, Role.DIRECTOR, Role.HR, Role.OPERATIONS_MANAGER, Role.TEAM_LEAD].includes(user.role)
  ...
  if (isManager) {
    ...
    activeTasks = await prisma.task.count({ where: { userId: user.id, status: { in: [TaskStatus.TODO, TaskStatus.IN_PROGRESS] } } })
  }
...
export async function updateTaskStatus(taskId: string, newStatus: string) {
  const user = await requireAuth()
  if (user.role === Role.CLIENT) throw new Error("Forbidden")
...
export async function applyLeave(formData: FormData) {
  ...
  await prisma.leave.create({ data: { userId: user.id, type, startDate, endDate, reason, status: LeaveStatus.PENDING } })
...
export async function assignTask(formData: FormData) {
  ...
  await prisma.task.create({ data: { title, description, priority, deadline, userId, assignedById: actor.id, projectId, status: TaskStatus.TODO } })
...
export async function createEmployee(formData: FormData) {
  ...
  const existing = await prisma.user.findFirst({ where: { OR: [{ email: emailAddress }, { employeeId }] }, select: { id: true } })
  if (existing) throw new Error("Email or employee ID already exists")
  const hashedPassword = await bcrypt.hash(rawPassword, 12)
  await prisma.user.create({ data: { name, email: emailAddress, employeeId, role, department, password: hashedPassword } })
  revalidatePath("/dashboard/operations")
}
...
export async function createProject(formData: FormData) {
  ...
  await prisma.project.create({ data: { name, description, clientName, startDate, endDate, status: ProjectStatus.PLANNING } })
...
export async function createTask(formData: FormData) {
  ...
  const description = optionalString(formData.get("description"), 5000)
  const priority = enumValue(formData.get("priority") || TaskPriority.MEDIUM, "Priority", TASK_PRIORITIES)
  await prisma.task.create({ data: { title, description, priority, userId: user.id, status: TaskStatus.TODO } })
  revalidatePath("/dashboard/tasks")
}
