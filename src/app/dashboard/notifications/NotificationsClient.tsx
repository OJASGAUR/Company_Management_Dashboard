...
export default function NotificationsClient({
  ...
  const filtered = useMemo(() => {
    switch (activeTab) {
      ...
      case "LEAVE":
        return notifications.filter((item) => item.type === "LEAVE")
      case "ALERT":
        return notifications.filter((item) => ["ALERT", "WARNING", "INFO"].includes(item.type))
      default:
  ...
  const typeStyle = (type: string) => {
    switch (type) {
      case "TASK":
        return { icon: "📝", label: "Task", badge: "bg-indigo-100 text-indigo-700", iconBg: "bg-indigo-50 border-indigo-100" }
      case "LEAVE":
        return { icon: "🏖️", label: "Leave / HR", badge: "bg-amber-100 text-amber-700", iconBg: "bg-amber-50 border-amber-100" }
      case "SUCCESS":
        return { icon: "✅", label: "Success", badge: "bg-emerald-100 text-emerald-700", iconBg: "bg-emerald-50 border-emerald-100" }
      case "ALERT":
      case "WARNING":
        return { icon: "⚠️", label: "Alert", badge: "bg-rose-100 text-rose-700", iconBg: "bg-rose-50 border-rose-100" }
      default:
        return { icon: "ℹ️", label: "Info", badge: "bg-blue-100 text-blue-700", iconBg: "bg-blue-50 border-blue-100" }
    }
  }

  const formatTimestamp = (value: string | Date) => {
    const date = new Date(value)
    // The relative timestamp is intentionally based on the current wall clock time.
  ...
  const submitBroadcast = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error(error)
      alert(error instanceof Error ? error.message : "Failed to broadcast notification.")
    } finally {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto border-b border-slate-200 pb-2">
        {tabs.map(([id, label]) => {
          const count = id === "ALL"
            ...
            : id === "UNREAD"
              ? unreadCount
              : notifications.filter((item) => id === "ALERT" ? ["ALERT", "WARNING", "INFO"].includes(item.type) : item.type === id).length
      ...
      {showSettings && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4"><form onSubmit={submitSettings} className="w-full max-w-md space-y-5 rounded-3xl bg-white p-7 shadow-2xl"><div><h2 className="text-xl font-extrabold text-slate-900">Notification Settings</h2><p className="mt-1 text-sm text-slate-500">Choose which emails you receive.</p></div>{[["emailTasks", "Tasks & Projects", userPreferences.emailTasks], ["emailLeaves", "Leaves & HR", userPreferences.emailLeaves], ["emailAnnouncements", "Company Announcements", userPreferences.emailAnnouncements]].map(([name, label, checked]) => <label key={name as string} className="flex items-center justify-between rounded-xl border border-slate-200 p-4"><span className="text-sm font-semibold text-slate-800">{label as string}</span><input type="checkbox" name={name as string} defaultChecked={checked as boolean} className="h-5 w-5" /></label>)}<div className="flex justify-end gap-2"><button type="button" onClick={() => setShowSettings(false)} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button disabled={savingSettings} type="submit" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{savingSettings ? "Saving..." : "Save preferences"}</button></div></form></div>}

      {showBroadcast && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4"><form onSubmit={submitBroadcast} className="w-full max-w-lg space-y-5 rounded-3xl bg-white p-7 shadow-2xl"><div><h2 className="text-xl font-extrabold text-slate-900">Broadcast Notification</h2><p className="mt-1 text-sm text-slate-500">Create an in-app notification and email active users who have enabled announcements.</p></div><input name="title" required maxLength={200} placeholder="Title" className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-500" /><select name="type" defaultValue="INFO" className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm"><option value="INFO">Information</option><option value="SUCCESS">Success</option><option value="ALERT">Alert</option><option value="WARNING">Warning</option><option value="TASK">Task</option><option value="LEAVE">Leave / HR</option></select><input name="link" placeholder="Optional portal link" maxLength={500} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-500" /><textarea name="message" required maxLength={4000} rows={6} placeholder="Message" className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-500" /><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowBroadcast(false)} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button disabled={broadcasting} type="submit" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{broadcasting ? "Sending..." : "Send broadcast"}</button></div></form></div>}
    </div>
  )
}
