...
async function main() {
  ...
  const project1 = await prisma.project.create({
    data: {
      name: "Global E-Commerce Redesign",
      description: "Overhauling the main e-commerce platform for better UX and conversion.",
  ...
  const project2 = await prisma.project.create({
    data: {
      ...
      description: "Building the internal leave and attendance management system.",
      clientName: "Internal",
      status: "PLANNING",
  ...
  // Give developer some attendance
  const now = new Date()
  await prisma.attendance.create({
    data: {
      userId: userIds['DEVELOPER'],
      date: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1),
      checkIn: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1, 9, 0, 0),
      checkOut: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1, 17, 30, 0),
      status: "PRESENT"
    }
  })

  // Give designer a pending leave
  ...
  // Give developer an approved leave
  await prisma.leave.create({
    data: {
      userId: userIds['DEVELOPER'],
      type: "SICK",
  ...
  await prisma.domain.create({ data: { url: "company-internal.com", provider: "AWS Route53", expiryDate: new Date(2027, 5, 1) }})
